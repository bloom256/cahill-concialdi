...
| `geo-stats-nov` (built) | Photographic Earth with the numbers | November Blue Marble plus white country borders and name and one line of numbers (population / GDP / GDP per capita) for every country: inside where it fits (5-14 pt on the 190 cm print), otherwise a 5 pt label right on the small country; countries under 1 million people show only their name in regular weight; leader lines only where that spot is taken (5 countries); island groups the country labels miss are named from `data/places.json` (Hawaii, the Line Islands) | yes | Tiny text needs close viewing or a larger print |
