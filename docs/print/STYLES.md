...
| `geo-blue-marble` (built) | Photographic Earth, early summer | NASA Blue Marble NG, June 2004 (owner's current favorite, after November), faint white graticule with the shared print grid (thin lines, grid labels); no fills or borders | yes (21600x10800 source, rendered at 150 dpi) | Imagery softer near face vertices; large file |
| `geo-stats-nov` (built) | Photographic Earth with the numbers | November Blue Marble plus white country borders and, for every country, a two-line block of ISO3 code and whole numbers (DEU/84M over $5T/$60K): inside where it fits; long thin countries turn their label along their own axis (PCA), and when tilted it becomes one line (PRT/10M/$346B/$33K); positions of all labels are then optimized jointly (7-14 pt on the 190 cm print), otherwise a 7 pt label right on the small country, shrinking to 6.5 or 6 pt rather than taking a leader line; countries under 1 million people show only their name in regular weight; leader lines only where that spot is taken, and countries under 300 000 people left off entirely rather than put on a line; island groups the country labels miss are named from `data/places.json` (Hawaii, the Line Islands) | yes | Tiny text needs close viewing or a larger print |
