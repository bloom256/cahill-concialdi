...
| `geo-stats-nov` (built) | Photographic Earth with the numbers | November Blue Marble plus white country borders and name, population, GDP and GDP per capita for every country: inside where it fits (2-10 pt, read up close), otherwise a 2 pt label right on the small country; leader lines only where that spot is taken (about 1) | yes | Tiny text needs close viewing or a larger print |
